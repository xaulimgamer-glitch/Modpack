(function () {
  ...
  function awakeningArchBowV2Model(type) {
    return {
      parent: 'item/generated', textures: { layer0: type.model.texture },
      display: {
  ...
  StartupEvents.registry('item', function (event) {
    ...
    data.materials.forEach(function (material) {
      ...
      Object.keys(data.bow_types).forEach(function (typeId) {
        ...
        event.create(itemId, 'bow').displayName(material.name + ' ' + type.name).maxDamage(type.max_damage).modelJson(awakeningArchBowV2Model(type)).bow(function (bow) {
