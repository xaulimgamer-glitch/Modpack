(function () {
  ...
  var AWAKENING_PLATE_UNIFICATION = [
    {
      id: 'iron',
  ...
  var awakeningPlateHeatedMetals = JsonIO.read('kubejs/awakening/heated_metals.json')
  var awakeningPlateArmor = JSON.parse(JsonIO.readString('kubejs/awakening/armor_forging.json'))
  ...
  if (awakeningPlateHeatedMetals && Array.isArray(awakeningPlateHeatedMetals.metals) &&
    ...
    var heatedById = {}
    awakeningPlateHeatedMetals.metals.forEach(function (metal) {
      if (metal && metal.id && metal.heated) heatedById[metal.id] = metal
    })

    awakeningPlateArmor.materials.forEach(function (material) {
      var heatedMetal = heatedById[material.id]
  ...
  ServerEvents.tags('item', function (event) {
    AWAKENING_PLATE_UNIFICATION.forEach(function (material) {
      material.tags.forEach(function (tag) {
        event.add(tag, material.canonical)
        material.deprecated.forEach(function (item) { event.remove(tag, item) })
      })
    })
  })
  ...
  ServerEvents.recipes(function (event) {
    AWAKENING_PLATE_UNIFICATION.forEach(function (material) {
      var hasInput = Ingredient.of(material.pressIngredient).itemIds.size() > 0
      var hasOutput = Ingredient.of({ item: material.canonical }).itemIds.size() > 0
      ...
      material.deprecated.forEach(function (item) { event.remove({ output: item }) })
